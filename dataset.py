    ...
    def load_data(self):
        ...
        if self.train:
            ...
            if self.classific:
                ...
                prediction_x = [prediction[k,i*96:96*(i+7)] for k in range(self.households) for i in np.arange(35136//96 - 7) not in self.inds]
                prediction_y = [prediction[k,(i+7)*96:96*(i+8)] for k in range(self.households) for i in np.arange(35136//96 - 7) not in self.inds]
        ...
        if self.classific:
            ...
            return torch.Tensor(np.array(prediction_x)).to(self.device), torch.Tensor(prediction_y).to(device = self.device, dtype = torch.long)
